...
private:
        ...
        // creates a new node for the insert function separately for modularity
        Node *newNodeHelper(int value);
        
        // Removes a node in a tree.
        // If the node is a duplicate it decrements its count_total.
        // If a node has two children it will replace that node with the 
        // smallest value of the right subtree.
        bool  remove(Node *node, Node *parent, int value);
        
        // helps determine which type of removal is needed in the main remove
        void removeHelper(Node *node, Node *parent);
        // handles removal of elements with no children
        void removeLeaf(Node *node, Node *parent);
        // handles removal of elements with one child
        void removeOneChild(Node *node, Node *parent);
        // handles removal of elements with two children 
        void removeTwoChildren(Node *node);
        // handles removal of elements on the left subtree
        void removeLeft(Node *node, Node *parent);
        // handles removal of elements on the right subtree
        void removeRight(Node *node, Node *parent);
        ...
        // creates a copy of a tree using preorder traversal
